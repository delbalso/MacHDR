...
- (float)getValueForControl:(uvc_control_info_t *)control {
    ...
    //NSLog(@"min is :%d, max is :%d, found value is %d",range.min,range.max,intval);
...
// Set a normalized value
- (BOOL)setExactValue:(float)value forControl:(uvc_control_info_t *)control {
	//// TODO: Cache the range somewhere?
	uvc_range_t range = [self getRangeForControl:control];
...
// Get a normalized value
- (float)getExactValueForControl:(uvc_control_info_t *)control {
	// TODO: Cache the range somewhere?
	//uvc_range_t range = [self getRangeForControl:control];
...
- (BOOL)setAutoExposure:(BOOL)enabled {
	int intval = (enabled ? 0x02 : 0x01); // "auto exposure modes" ar NOT boolean (on|off) as it seems
    [self setData:intval 
			  withLength:uvc_controls.autoExposure.size 
			 forSelector:uvc_controls.autoExposure.selector 
					  at:uvc_controls.autoExposure.unit];
    NSLog(@"EXPOSURE from Auto IS %f", [self getExposure]);
    return;
}
...
- (BOOL)getAutoExposure {
	...
	return ( intval == 0x02 ? YES : NO );
}

- (BOOL)setExposure:(float)value {
	value = 1 - value;
    [self setValue:value forControl:&uvc_controls.exposure];
    NSLog(@"EXPOSURE IS %f", [self getExposure]);
    return;
	//return [self setValue:value forControl:&uvc_controls.exposure];
}

- (BOOL)setExactExposure:(float)value {
    [self setExactValue:value forControl:&uvc_controls.exposure];
    NSLog(@"EXPOSURE IS %f microseconds", [self getExactExposure]);
    return;
	//return [self setValue:value forControl:&uvc_controls.exposure];
}
...
- (float)getExactExposure {
	float value = [self getExactValueForControl:&uvc_controls.exposure];
	return value;
}
